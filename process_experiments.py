...
from expt_config_parser_TEMP import parseExptConfig, parseLibraryConfig
...
def processExperimentsFromConfig(configFile, libraryDirectory):
    ...
    for tup in sorted(exptParameters['counts_file_list']):
        ...
        countSeries = readCountsFile(tup[2])
        countSeries = libraryTable.align(countSeries, axis=0, join='outer', fill_value=0)[1] #expand series to fill 0 for every missing entry

        columnDict[tup] = countSeries[sublibColumn] #then shrink series to only desired sublibraries
